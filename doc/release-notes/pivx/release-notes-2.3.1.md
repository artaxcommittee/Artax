...
| --- | --- | --- |
| `merchantnode count` | `getmerchantnodecount` | |
| `merchantnode list` | `listmerchantnodes` | |
| `merchantnodelist` | `listmerchantnodes` | renamed |
| `merchantnode connect` | `merchantnodeconnect` | |
| `merchantnode current` | `getcurrentmerchantnode` | |
| `merchantnode debug` | `merchantnodedebug` | |
| `merchantnode enforce` |  | removed |
| `merchantnode outputs` | `getmerchantnodeoutputs` | |
| `merchantnode status` | `getmerchantnodestatus` | |
| `merchantnode list-conf` | `listmerchantnodeconf` | added optional filter |
| `merchantnode genkey` | `createmerchantnodekey` | |
| `merchantnode winners` | `listmerchantnodewinners` | |
| `merchantnode start` | `startmerchantnode` | see notes below |
| `merchantnode start-alias` | `startmerchantnode` | see notes below |
| `merchantnode start-<mode>` | `startmerchantnode` | see notes below |
| `merchantnode create` | | removed - not implemented |
| `merchantnode calcscore` | `listmerchantnodescores` | |
| --- | --- | --- |
...
##### `startmerchantnode` Command #####
This command now handles all cases for starting a merchantnode instead of having multiple commands based on the context. Command arguments have changed slightly to allow the user to decide wither or not to re-lock the wallet after the command is run. Below is the help documentation:
...
Examples:
> pivx-cli startmerchantnode "alias" true "my_mn"
> curl --user myusername --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "startmerchantnode", "params": ["alias" true "my_mn"] }' -H 'content-type: text/plain;' http://127.0.0.1:9332/
```
...
#### Merchantnode network protocol layer reporting ####
The results from the `listmerchantnodes` and `getmerchantnodecount` commands now includes details about which network protocol layer is being used (IPv4, IPV6, or Tor).
...
### RPC and other APIs
- #239 `e8b92f4` [RPC] Make 'merchantnode status' more verbose (Mrs-X)
