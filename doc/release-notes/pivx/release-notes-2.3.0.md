...
The `merchantnode` RPC command has been re-worked to ease it's usage and return valid JSON in it's results. The following is an overview of the changed command parameters:
...
| `list` | Remove all optional "modes" and standardized the results. Note: `merchantnode list` is the same as `merchantnodelist`. See below |
...
For the `list` (aka `merchantnodelist`) parameter, the various "modes" have been removed in favor of a unified and standardized result format. The result is now an array of objects instead of an object of objects. Further, the individual objects now have a standard JSON format. The result format is as follows:
...
[
  {
    ...
    "addr": addr,      (string) Merchantnode PIVX address
    "version": v,      (numeric) Merchantnode Protocol version
    "lastseen": ttt,   (numeric) The time in seconds since epoch (Jan 1 1970 GMT) the merchantnode was last seen
...
**All** Merchantnodes List GUI Removal
--------------

With the standardization and reformatting of the `merchantnode list` (`merchantnodelist`) RPC command, there is no real use case to keep the full list of merchantnodes in the GUI. This GUI element causes a great deal of extra overhead, even when it is not being actively displayed. The removal of this list has also proven to resolve a number of linux-based errors

Note that the GUI list of merchantnodes associated with a controller wallet remains intact.
...
- #211 `b8c110b` [RPC] Refactor & JSONify results from merchantnode command(s) (Fuzzbawls)
...
- #206 `9c675ee` [Qt] Remove the All Merchantnodes UI tab/list (Fuzzbawls)
...
- #213 `ddd8994` [Trivial] Reduce debug.log spam for merchantnode messages (Fuzzbawls)
