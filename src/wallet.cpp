...
/**
 * Fees smaller than this (in uxax) are considered zero fee (for transaction creation)
...
bool CWallet::GetMerchantnodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    if (vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetMerchantnodeVinAndKeys -- Could not locate any valid merchantnode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetMerchantnodeVinAndKeys -- Could not locate specified merchantnode vin\n");
    return false;
}
...
CAmount CWalletTx::GetUnlockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && vout[i].nValue == MERCHANTNODE_COLLATERAL * COIN) continue; // do not count MN-like outputs
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add merchantnode collaterals which are handled like locked coins
        else if (fMasterNode && vout[i].nValue == MERCHANTNODE_COLLATERAL * COIN) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
...
CAmount CWalletTx::GetLockedWatchOnlyCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add merchantnode collaterals which are handled likc locked coins
        else if (fMasterNode && vout[i].nValue == MERCHANTNODE_COLLATERAL * COIN) {
            nCredit += pwallet->GetCredit(txout, ISMINE_WATCH_ONLY);
        }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_NOT10000IFMN) {
                    found = !(fMasterNode && pcoin->vout[i].nValue == MERCHANTNODE_COLLATERAL * COIN);
                } if (nCoinType == ONLY_10000) {
                    found = pcoin->vout[i].nValue == MERCHANTNODE_COLLATERAL * COIN;
                } else {
                    found = true;
                }
                if (!found) continue;
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 XAX.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-xax-address
                    CScript scriptChange;
...
void CWallet::AutoCombineDust()
{
    ...
    for (map<CBitcoinAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
        ...
        //find merchantnode rewards that need to be combined
...
bool CWallet::MultiSend()
{
    ...
    for (const COutput& out : vCoins) {
        ...
        bool sendMSonMNReward = fMultiSendMerchantnodeReward && outpoint.IsMerchantnodeReward(out.tx);
        ...
        if (!walletdb.WriteMSettings(fMultiSendStake, fMultiSendMerchantnodeReward, nLastMultiSendHeight))
        ...
        if ((stakeSent && mnSent) || (stakeSent && !fMultiSendMerchantnodeReward) || (mnSent && !fMultiSendStake))
            return true;
    }
