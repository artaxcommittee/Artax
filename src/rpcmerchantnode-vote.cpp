...
UniValue submitcommunityproposal(const UniValue& params, bool fHelp)
{
    ...
    communityVote.mapSeenMerchantnodeCommunityProposals.insert(make_pair(communityProposalBroadcast.GetHash(), communityProposalBroadcast));
...
UniValue getcommunityinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nShow current merchantnode community proposals\n"
...
UniValue mncommunityvote(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "local") {
        ...
        while (true) {
            ...
            CCommunityVote vote(activeMerchantnode.vin, hash, nVote);
            ...
            if (communityVote.UpdateProposal(vote, NULL, strError)) {
                success++;
                communityVote.mapSeenMerchantnodeCommunityVotes.insert(make_pair(vote.GetHash(), vote));
                vote.Relay();
                statusObj.push_back(Pair("node", "local"));
