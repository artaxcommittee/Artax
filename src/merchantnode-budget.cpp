...
void CBudgetManager::CheckOrphanVotes()
{
    ...
    std::string strError = "";
    std::map<uint256, CBudgetVote>::iterator it1 = mapOrphanMerchantnodeBudgetVotes.begin();
    while (it1 != mapOrphanMerchantnodeBudgetVotes.end()) {
        if (budget.UpdateProposal(((*it1).second), NULL, strError)) {
            ...
            mapOrphanMerchantnodeBudgetVotes.erase(it1++);
        } else {
            ++it1;
        }
    }
...
void CBudgetManager::SubmitFinalBudget()
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        // NOTE: 9 blocks for testnet is way to short to have any merchantnode submit an automatic vote on the finalized(!) budget,
...
CBudgetDB::CBudgetDB()
{
    ...
    strMagicMessage = "MerchantnodeBudget";
}
...
bool CBudgetManager::IsBudgetPaymentBlock(int nBlockHeight)
{
    ...
    LogPrint("mnbudget","CBudgetManager::IsBudgetPaymentBlock() - nHighestCount: %lli, 5%% of Merchantnodes: %lli. Number of finalized budgets: %lli\n", 
              nHighestCount, nFivePercent, mapFinalizedBudgets.size());

    // If budget doesn't have 5% of the network votes, then we should pay a merchantnode instead
...
TrxValidationStatus CBudgetManager::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    LogPrint("mnbudget","CBudgetManager::IsTransactionValid() - nHighestCount: %lli, 5%% of Merchantnodes: %lli mapFinalizedBudgets.size(): %ld\n",
              nHighestCount, nFivePercent, mapFinalizedBudgets.size());
    /*
        If budget doesn't have 5% of the network votes, then we should pay a merchantnode instead
    */
    ...
    // If not enough merchantnodes autovoted for any of the finalized budgets pay a merchantnode instead
    if(!fThreshold) {
...
void CBudgetManager::NewBlock()
{
    ...
    if (merchantnodeSync.RequestedMerchantnodeAssets <= MERCHANTNODE_SYNC_BUDGET) return;
...
void CBudgetManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnvs") { //Merchantnode vote sync
        uint256 nProp;
        vRecv >> nProp;
        ...
        Sync(pfrom, nProp);
        LogPrint("mnbudget", "mnvs - Sent Merchantnode votes to peer %i\n", pfrom->GetId());
    }

    if (strCommand == "mprop") { //Merchantnode Proposal
        CBudgetProposalBroadcast budgetProposalBroadcast;
        vRecv >> budgetProposalBroadcast;

        if (mapSeenMerchantnodeBudgetProposals.count(budgetProposalBroadcast.GetHash())) {
            merchantnodeSync.AddedBudgetItem(budgetProposalBroadcast.GetHash());
            return;
        }
        ...
        mapSeenMerchantnodeBudgetProposals.insert(make_pair(budgetProposalBroadcast.GetHash(), budgetProposalBroadcast));
    ...
    if (strCommand == "mvote") { //Merchantnode Vote
        CBudgetVote vote;
        vRecv >> vote;
        vote.fValid = true;

        if (mapSeenMerchantnodeBudgetVotes.count(vote.GetHash())) {
            merchantnodeSync.AddedBudgetItem(vote.GetHash());
            return;
        }

        CMerchantnode* pmn = mnodeman.Find(vote.vin);
        if (pmn == NULL) {
            LogPrint("mnbudget","mvote - unknown merchantnode - vin: %s\n", vote.vin.prevout.hash.ToString());
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }


        mapSeenMerchantnodeBudgetVotes.insert(make_pair(vote.GetHash(), vote));
    ...
    if (strCommand == "fbs") { //Finalized Budget Suggestion
        ...
        if (mapSeenFinalizedBudgets.count(finalizedBudgetBroadcast.GetHash())) {
            merchantnodeSync.AddedBudgetItem(finalizedBudgetBroadcast.GetHash());
            return;
        }
    ...
    if (strCommand == "fbvote") { //Finalized Budget Vote
        ...
        CMerchantnode* pmn = mnodeman.Find(vote.vin);
        if (pmn == NULL) {
            LogPrint("mnbudget", "fbvote - unknown merchantnode - vin: %s\n", vote.vin.prevout.hash.ToString());
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }
...
void CBudgetManager::ResetSync()
{
    ...
    std::map<uint256, CBudgetProposalBroadcast>::iterator it1 = mapSeenMerchantnodeBudgetProposals.begin();
    while (it1 != mapSeenMerchantnodeBudgetProposals.end()) {
...
void CBudgetManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    */
    ...
    pfrom->PushMessage("ssc", MERCHANTNODE_SYNC_BUDGET_PROP, nInvCount);
    ...
    pfrom->PushMessage("ssc", MERCHANTNODE_SYNC_BUDGET_FIN, nInvCount);
    LogPrint("mnbudget", "CBudgetManager::Sync - sent %d items\n", nInvCount);
}
...
bool CBudgetManager::UpdateProposal(CBudgetVote& vote, CNode* pfrom, std::string& strError)
{
    ...
    if (!mapProposals.count(vote.nProposalHash)) {
        if (pfrom) {
            ...
            mapOrphanMerchantnodeBudgetVotes[vote.nProposalHash] = vote;
...
bool CBudgetVote::Sign(CKey& keyMerchantnode, CPubKey& pubKeyMerchantnode)
{
    // Choose coins to use
...
bool CBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        if (fDebug){
            LogPrint("mnbudget","CBudgetVote::SignatureValid() - Unknown Merchantnode - %s\n", vin.prevout.hash.ToString());
        }
        return false;
    }
...
//evaluate if we should vote for this. Merchantnode only
...
bool CFinalizedBudget::IsPaidAlready(uint256 nProposalHash, int nBlockHeight)
{
    ...
    // This budget was paid already -> reject transaction so it gets paid to a merchantnode instead
    return true;
}
...
TrxValidationStatus CFinalizedBudget::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    BOOST_FOREACH (CTxOut out, txNew.vout) {
        ...
        if (vecBudgetPayments[nCurrentBudgetPayment].payee == out.scriptPubKey && vecBudgetPayments[nCurrentBudgetPayment].nAmount == out.nValue) {
            // Check if this proposal was paid already. If so, pay a merchantnode instead
            paid = IsPaidAlready(vecBudgetPayments[nCurrentBudgetPayment].nProposalHash, nBlockHeight);
            if(paid) {
                LogPrint("mnbudget","CFinalizedBudget::IsTransactionValid - Double Budget Payment of %d for proposal %d detected. Paying a merchantnode instead.\n",
...
void CFinalizedBudget::SubmitVote()
{
    ...
    CFinalizedBudgetVote vote(activeMerchantnode.vin, GetHash());
    if (!vote.Sign(keyMerchantnode, pubKeyMerchantnode)) {
...
bool CFinalizedBudgetVote::Sign(CKey& keyMerchantnode, CPubKey& pubKeyMerchantnode)
{
    // Choose coins to use
    CPubKey pubKeyCollateralAddress;
    CKey keyCollateralAddress;
...
bool CFinalizedBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        LogPrint("mnbudget","CFinalizedBudgetVote::SignatureValid() - Unknown Merchantnode %s\n", strMessage);
        return false;
    }
...
std::string CBudgetManager::ToString() const
{
    ...
    info << "Proposals: " << (int)mapProposals.size() << ", Budgets: " << (int)mapFinalizedBudgets.size() << ", Seen Budgets: " << (int)mapSeenMerchantnodeBudgetProposals.size() << ", Seen Budget Votes: " << (int)mapSeenMerchantnodeBudgetVotes.size() << ", Seen Final Budgets: " << (int)mapSeenFinalizedBudgets.size() << ", Seen Final Budget Votes: " << (int)mapSeenFinalizedBudgetVotes.size();
