...
void MultiSendDialog::updateCheckBoxes()
{
    ...
    ui->multiSendMerchantnodeCheckBox->setChecked(pwalletMain->fMultiSendMerchantnodeReward);
}
...
void MultiSendDialog::on_viewButton_clicked()
{
    ...
    if (pwalletMain->isMultiSendEnabled()) {
        if (pwalletMain->fMultiSendStake && pwalletMain->fMultiSendMerchantnodeReward)
            strMultiSendPrint += "MultiSend Active for Stakes and Merchantnode Rewards\n";
        else if (pwalletMain->fMultiSendStake)
            strMultiSendPrint += "MultiSend Active for Stakes\n";
        else if (pwalletMain->fMultiSendMerchantnodeReward)
            strMultiSendPrint += "MultiSend Active for Merchantnode Rewards\n";
    } else
...
void MultiSendDialog::on_activateButton_clicked()
{
    ...
    if (pwalletMain->vMultiSend.size() < 1)
    ...
    else if (!(ui->multiSendStakeCheckBox->isChecked() || ui->multiSendMerchantnodeCheckBox->isChecked())) {
        strRet = "Need to select to send on stake and/or merchantnode rewards\n";
        ...
        pwalletMain->fMultiSendMerchantnodeReward = ui->multiSendMerchantnodeCheckBox->isChecked();

        CWalletDB walletdb(pwalletMain->strWalletFile);
        if (!walletdb.WriteMSettings(pwalletMain->fMultiSendStake, pwalletMain->fMultiSendMerchantnodeReward, pwalletMain->nLastMultiSendHeight))
