...
enum class TrxValidationStatus {
    ...
    VoteThreshold    /** If not enough merchantnodes have voted on a finalized budget */
};
...
//
// CBudgetVote - Allow a merchantnode node to vote and broadcast throughout the network
//
...
//
// CFinalizedBudgetVote - Allow a merchantnode node to vote and broadcast throughout the network
//
...
public:
    ...
    std::map<uint256, CBudgetProposalBroadcast> mapSeenMerchantnodeBudgetProposals;
    std::map<uint256, CBudgetVote> mapSeenMerchantnodeBudgetVotes;
    std::map<uint256, CBudgetVote> mapOrphanMerchantnodeBudgetVotes;
    ...
    void ClearSeen()
    {
        mapSeenMerchantnodeBudgetProposals.clear();
        mapSeenMerchantnodeBudgetVotes.clear();
    ...
    void Clear()
    {
        ...
        mapSeenFinalizedBudgets.clear();
        mapSeenFinalizedBudgetVotes.clear();
        mapOrphanMerchantnodeBudgetVotes.clear();
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion)
    {
        READWRITE(mapSeenMerchantnodeBudgetProposals);
        READWRITE(mapSeenMerchantnodeBudgetVotes);
        READWRITE(mapSeenFinalizedBudgets);
        READWRITE(mapSeenFinalizedBudgetVotes);
        READWRITE(mapOrphanMerchantnodeBudgetVotes);
...
private:
    // critical section to protect the inner data structures
    mutable CCriticalSection cs;
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (merchantnode only)
...
public:
    ...
    void AutoCheck();
    //total xax paid out by this budget
    CAmount GetTotalPayout();
    //vote on this finalized budget as a merchantnode
    void SubmitVote();
...
//
// Budget Proposal : Contains the merchantnode votes for each budget
//
