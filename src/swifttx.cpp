...
//step 2.) Top SWIFTTX_SIGNATURES_TOTAL merchantnodes, open connect to top 1 merchantnode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 merchantnode, waits for SWIFTTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
...
void ProcessMessageSwiftTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    } else if (strCommand == "txlvote") // SwiftTX Lock Consensus Votes
    {
        ...
        if (ProcessConsensusVote(pfrom, ctx)) {
            //Spam/Dos protection
            /*
                Merchantnodes will sometimes propagate votes before the transaction is known to the client.
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                if (!mapUnknownVotes.count(ctx.vinMerchantnode.prevout.hash)) {
                    mapUnknownVotes[ctx.vinMerchantnode.prevout.hash] = GetTime() + (60 * 10);
                }

                if (mapUnknownVotes[ctx.vinMerchantnode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinMerchantnode.prevout.hash] - GetAverageVoteTime() > 60 * 10) {
                    LogPrintf("ProcessMessageSwiftTX::ix - merchantnode is spamming transaction votes: %s %s\n",
                        ctx.vinMerchantnode.ToString().c_str(),
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which merchantnodes
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetMerchantnodeRank(activeMerchantnode.vin, nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    if (n == -1) {
        LogPrint("swifttx", "SwiftTX::DoConsensusVote - Unknown Merchantnode\n");
        return;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swifttx", "SwiftTX::DoConsensusVote - Merchantnode not in the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinMerchantnode = activeMerchantnode.vin;
...
{
    int n = mnodeman.GetMerchantnodeRank(ctx.vinMerchantnode, ctx.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    CMerchantnode* pmn = mnodeman.Find(ctx.vinMerchantnode);
    if (pmn != NULL)
        LogPrint("swifttx", "SwiftTX::ProcessConsensusVote - Merchantnode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("swifttx", "SwiftTX::ProcessConsensusVote - Unknown Merchantnode\n");
        mnodeman.AskForMN(pnode, ctx.vinMerchantnode);
        return false;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swifttx", "SwiftTX::ProcessConsensusVote - Merchantnode not in the top %d (%d) - %s\n", SWIFTTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("SwiftTX::ProcessConsensusVote - Signature invalid\n");
        // don't ban, it could just be a non-synced merchantnode
        mnodeman.AskForMN(pnode, ctx.vinMerchantnode);
        return false;
    }
...
{
    return vinMerchantnode.prevout.hash + vinMerchantnode.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("SwiftTX::CConsensusVote::SignatureValid() - Unknown Merchantnode\n");
        return false;
    }

    if (!merchantnodeSigner.VerifyMessage(pmn->pubKeyMerchantnode, vchMasterNodeSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    if (!merchantnodeSigner.SetKey(strMasterNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid merchantnodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if (!merchantnodeSigner.SignMessage(strMessage, errorMessage, vchMasterNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if (!merchantnodeSigner.VerifyMessage(pubkey2, vchMasterNodeSignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetMerchantnodeRank(vote.vinMerchantnode, vote.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

        if (n == -1) {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Merchantnode\n");
            return false;
        }

        if (n > SWIFTTX_SIGNATURES_TOTAL) {
            LogPrintf("CTransactionLock::SignaturesValid() - Merchantnode not in the top %s\n", SWIFTTX_SIGNATURES_TOTAL);
            return false;
        }
