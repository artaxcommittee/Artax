...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "artax.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "artaxd.pid"));
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in XAX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in XAX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-rescan", _("Rescan the block chain for missing wallet transactions") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-artaxwallet", _("Attempt to recover private keys from a corrupt wallet.dat") + " " + _("on startup"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, artax, (swifttx, merchantnode, mnpayments, mnbudget, mncommunityvote)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in XAX/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Artax specific functionality (Merchantnodes, SwiftTX, Budgeting, Community Votes) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageGroup(_("Merchantnode options:"));
    strUsage += HelpMessageOpt("-merchantnode=<n>", strprintf(_("Enable the client to act as a merchantnode (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify merchantnode configuration file (default: %s)"), "merchantnode.conf"));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf(_("Lock merchantnodes from merchantnode configuration file (default: %u)"), 1));
    strUsage += HelpMessageOpt("-merchantnodeprivkey=<n>", _("Set the merchantnode private key"));
    strUsage += HelpMessageOpt("-merchantnodeaddr=<n>", strprintf(_("Set external address:port to get to this merchantnode (example: %s)"), "128.127.106.235:9333"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Artax Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Artax is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (GetBoolArg("-artaxwallet", false)) {
        // Rewrite just private keys: rescan to find transactions
        if (SoftSetBoolArg("-rescan", true))
            LogPrintf("AppInit2 : parameter interaction: -artaxwallet=1 -> setting -rescan=1\n");
    }
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Artax Core is shutting down."));
...
#endif
    // Make sure only a single Artax process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Artax Core is probably already running."), strDataDir));
    ...
    if (!fDisableWallet) {
        ...
        if (filesystem::exists(GetDataDir() / strWalletFile)) {
            ...
            if (r == CDBEnv::RECOVER_OK) {
                string msg = strprintf(_("Warning: wallet.dat corrupt, data artaxd!"
            ...
            if (r == CDBEnv::RECOVER_FAIL)
                return InitError(_("wallet.dat corrupt, artax failed"));
        }
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Artax: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Artax Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Artax Core to complete") << "\n";
    ...
    // ********************************************************* Step 10: setup Merchantnode

    uiInterface.InitMessage(_("Loading merchantnode cache..."));

    CMerchantnodeDB mndb;
    CMerchantnodeDB::ReadResult readResult = mndb.Read(mnodeman);
    if (readResult == CMerchantnodeDB::FileError)
        LogPrintf("Missing merchantnode cache file - mncache.dat, will try to recreate\n");
    ...
    uiInterface.InitMessage(_("Loading merchantnode payment cache..."));

    CMerchantnodePaymentDB mnpayments;
    CMerchantnodePaymentDB::ReadResult readResult3 = mnpayments.Read(merchantnodePayments);

    if (readResult3 == CMerchantnodePaymentDB::FileError)
        LogPrintf("Missing merchantnode payment cache - mnpayments.dat, will try to recreate\n");
    else if (readResult3 != CMerchantnodePaymentDB::Ok) {
        LogPrintf("Error reading mnpayments.dat: ");
        if (readResult3 == CMerchantnodePaymentDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }
    ...
    fMasterNode = GetBoolArg("-merchantnode", false);

    if ((fMasterNode || merchantnodeConfig.getCount() > -1) && fTxIndex == false) {
        return InitError("Enabling Merchantnode support requires turning on transaction indexing."
                         "Please add txindex=1 to your configuration and start with -reindex");
    }

    if (fMasterNode) {
        LogPrintf("IS MASTER NODE\n");
        strMasterNodeAddr = GetArg("-merchantnodeaddr", "");
        ...
        if (!strMasterNodeAddr.empty()) {
            ...
            if (!addrTest.IsValid()) {
                return InitError("Invalid -merchantnodeaddr address: " + strMasterNodeAddr);
            }
        }

        strMasterNodePrivKey = GetArg("-merchantnodeprivkey", "");
        if (!strMasterNodePrivKey.empty()) {
            ...
            if (!merchantnodeSigner.SetKey(strMasterNodePrivKey, errorMessage, key, pubkey)) {
                return InitError(_("Invalid merchantnodeprivkey. Please see documenation."));
            }

            activeMerchantnode.pubKeyMerchantnode = pubkey;
    ...
    //get the mode of budget voting for this merchantnode
    strBudgetMode = GetArg("-budgetvotemode", "auto");

    if (GetBoolArg("-mnconflock", true) && pwalletMain) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking Merchantnodes:\n");
        uint256 mnTxHash;
    ...
    //lite mode disables all Merchantnode related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if (fMasterNode && fLiteMode) {
        return InitError("You can not start a merchantnode in litemode");
    }
    ...
    merchantnodeSigner.InitCollateralAddress();

    threadGroup.create_thread(boost::bind(&ThreadMerchantnodePool));
