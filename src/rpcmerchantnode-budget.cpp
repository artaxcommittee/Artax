...
UniValue mnbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-alias         - Vote on a Artax initiative\n"
            "  vote               - Vote on a Artax initiative/budget\n"
            "  getvotes           - Show current merchantnode budgets\n"
            "  getinfo            - Show current merchantnode budgets\n"
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"xax-address\" monthy-payment\n"
            ...
            "5. \"xax-address\":   (string, required) XAX address to send payments to\n"
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 7)
        throw runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"xax-address\" monthy-payment \"fee-tx\"\n"
    ...
    if (!merchantnodeSync.IsBlockchainSynced()) {
        throw runtime_error("Must wait for client to sync with merchantnode network. Try again in a minute or so.");
    }
    ...
    budget.mapSeenMerchantnodeBudgetProposals.insert(make_pair(budgetProposalBroadcast.GetHash(), budgetProposalBroadcast));
...
UniValue mnbudgetvote(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() == 3 && (strCommand != "local" && strCommand != "many")) || (params.size() == 4 && strCommand != "alias") ||
        ...
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"mode\"      (string, required) The voting mode. 'local' for voting directly from a merchantnode, 'many' for voting with a MN controller and casting the same vote for each MN, 'alias' for voting with a MN controller and casting a vote for a single MN\n"
    ...
    if (strCommand == "local") {
        ...
        while (true) {
            if (!merchantnodeSigner.SetKey(strMasterNodePrivKey, errorMessage, keyMerchantnode, pubKeyMerchantnode)) {
                ...
                statusObj.push_back(Pair("error", "Merchantnode signing error, could not set key correctly: " + errorMessage));
            ...
            if (pmn == NULL) {
                ...
                statusObj.push_back(Pair("node", "local"));
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("error", "Failure to find merchantnode in list : " + activeMerchantnode.vin.ToString()));
                resultsObj.push_back(statusObj);
                break;
            }

            CBudgetVote vote(activeMerchantnode.vin, hash, nVote);
            ...
            if (budget.UpdateProposal(vote, NULL, strError)) {
                success++;
                budget.mapSeenMerchantnodeBudgetVotes.insert(make_pair(vote.GetHash(), vote));
                vote.Relay();
                statusObj.push_back(Pair("node", "local"));
    ...
    if (strCommand == "many") {
        BOOST_FOREACH (CMerchantnodeConfig::CMerchantnodeEntry mne, merchantnodeConfig.getEntries()) {
            ...
            if (!merchantnodeSigner.SetKey(mne.getPrivKey(), errorMessage, keyMerchantnode, pubKeyMerchantnode)) {
                failed++;
            ...
            CMerchantnode* pmn = mnodeman.Find(pubKeyMerchantnode);
            if (pmn == NULL) {
                failed++;
                statusObj.push_back(Pair("node", mne.getAlias()));
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("error", "Can't find merchantnode by pubkey"));
                resultsObj.push_back(statusObj);
                continue;
            }
...
UniValue getbudgetvotes(const UniValue& params, bool fHelp)
{
    if (params.size() != 1)
        throw runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"mnId\": \"xxxx\",        (string) Hash of the merchantnode's collateral transaction\n"
...
UniValue getbudgetprojection(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) XAX address of payment\n"
...
UniValue getbudgetinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nShow current merchantnode budgets\n"
...
UniValue mnbudgetrawvote(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "mnbudgetrawvote \"merchantnode-tx-hash\" merchantnode-tx-index \"proposal-hash\" yes|no time \"vote-sig\"\n"
            "\nCompile and relay a proposal vote with provided external signature instead of signing vote internally\n"

            "\nArguments:\n"
            "1. \"merchantnode-tx-hash\"  (string, required) Transaction hash for the merchantnode\n"
            "2. merchantnode-tx-index   (numeric, required) Output index for the merchantnode\n"
    ...
    if (pmn == NULL) {
        return "Failure to find merchantnode in list : " + vin.ToString();
    }
...
UniValue mnfinalbudget(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "vote-many") {
        ...
        BOOST_FOREACH (CMerchantnodeConfig::CMerchantnodeEntry mne, merchantnodeConfig.getEntries()) {
            ...
            if (!merchantnodeSigner.SetKey(mne.getPrivKey(), errorMessage, keyMerchantnode, pubKeyMerchantnode)) {
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Merchantnode signing error, could not set key correctly: " + errorMessage));
            ...
            if (pmn == NULL) {
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Can't find merchantnode by pubkey"));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }
    ...
    if (strCommand == "vote") {
        ...
        CMerchantnode* pmn = mnodeman.Find(activeMerchantnode.vin);
        if (pmn == NULL) {
            return "Failure to find merchantnode in list : " + activeMerchantnode.vin.ToString();
        }

        CFinalizedBudgetVote vote(activeMerchantnode.vin, hash);
        if (!vote.Sign(keyMerchantnode, pubKeyMerchantnode)) {
