...
//presstab HyperStake
bool CWalletDB::WriteMSettings(bool fMultiSendStake, bool fMultiSendMerchantnode, int nLastMultiSendHeight)
{
    nWalletDBUpdated++;
    std::pair<bool, bool> enabledMS(fMultiSendStake, fMultiSendMerchantnode);
...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, string& strType, string& strErr)
{
    try {
        ...
        } else if (strType == "msettingsv2") //presstab HyperStake
        {
            ...
            pwallet->fMultiSendMerchantnodeReward = pSettings.first.second;
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call Artax with fAggressive=true to
    // get as much data as possible.
    // Rewrite artaxd data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> artaxdData;
    bool allOK = dbenv.Artax(newFilename, true, artaxdData);
    if (artaxdData.empty()) {
        LogPrintf("Artax(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("Artax(aggressive) found %u records\n", artaxdData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, artaxdData) {
        if (fOnlyKeys) {
