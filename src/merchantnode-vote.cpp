...
bool IsCommunityCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf)
{
    ...
    if (!GetTransaction(nTxCollateralHash, txCollateral, nBlockHash, true)) {
        ...
        LogPrint("merchantnode", "::IsCommunityCollateralValid - %s\n", strError);
        return false;
    }
...
void CCommunityVoteManager::CheckOrphanVotes()
{
    ...
    std::string strError = "";
    std::map<uint256, CCommunityVote>::iterator it1 = mapOrphanMerchantnodeCommunityVotes.begin();
    while (it1 != mapOrphanMerchantnodeCommunityVotes.end()) {
        if (communityVote.UpdateProposal(((*it1).second), NULL, strError)) {
            LogPrint("merchantnode", "CCommunityVoteManager::CheckOrphanVotes - Proposal/CommunityVote is known, activating and removing orphan vote\n");
            mapOrphanMerchantnodeCommunityVotes.erase(it1++);
        } else {
            ++it1;
        }
    }

    LogPrint("merchantnode", "CCommunityVoteManager::CheckOrphanVotes - Done\n");
}
...
CCommunityDB::CCommunityDB()
{
    ...
    strMagicMessage = "MerchantnodeCommunityVote";
}
...
bool CCommunityDB::Write(const CCommunityVoteManager& objToSave)
{
    ...
    LogPrint("merchantnode", "Written info to communityvote.dat  %dms\n", GetTimeMillis() - nStart);
...
CCommunityDB::ReadResult CCommunityDB::Read(CCommunityVoteManager& objToLoad, bool fDryRun)
{
    ...
    LogPrint("merchantnode", "Loaded info from communityvote.dat  %dms\n", GetTimeMillis() - nStart);
    LogPrint("merchantnode", "  %s\n", objToLoad.ToString());
    if (!fDryRun) {
        LogPrint("merchantnode", "Community vote manager - cleaning....\n");
        objToLoad.CheckAndRemove();
        LogPrint("merchantnode", "Community vote manager - result:\n");
        LogPrint("merchantnode", "  %s\n", objToLoad.ToString());
    }
...
void DumpCommunityVotes()
{
    ...
    LogPrint("merchantnode", "Verifying communityvote.dat format...\n");
    CCommunityDB::ReadResult readResult = votedb.Read(tempCommunityVote, true);
    // there was an error and it was not an error on file opening => do not proceed
    if (readResult == CCommunityDB::FileError)
        LogPrint("merchantnode", "Missing communityvote file - communityvote.dat, will try to recreate\n");
    else if (readResult != CCommunityDB::Ok) {
        LogPrint("merchantnode", "Error reading communityvote.dat: ");
    ...
    LogPrint("merchantnode", "Writting info to communityvote.dat...\n");
    votedb.Write(communityVote);

    LogPrint("merchantnode", "Community vote dump finished  %dms\n", GetTimeMillis() - nStart);
}
...
bool CCommunityVoteManager::AddProposal(CCommunityProposal& voteProposal)
{
    ...
    if (!voteProposal.IsValid(strError)) {
        LogPrint("merchantnode", "CCommunityVoteManager::AddProposal - invalid vote proposal - %s\n", strError);
        return false;
    }
    ...
    LogPrint("merchantnode", "CCommunityVoteManager::AddProposal - proposal %s added\n", voteProposal.GetName().c_str());
    return true;
}
...
void CCommunityVoteManager::CheckAndRemove()
{
    ...
    while (it2 != mapProposals.end()) {
        ...
        if (!strError.empty()) {
            LogPrint("merchantnode", "CCommunityVoteManager::CheckAndRemove - Invalid vote proposal - %s\n", strError);
            strError = "";
        } else {
             LogPrint("merchantnode", "CCommunityVoteManager::CheckAndRemove - Found valid vote proposal: %s %s\n",
    ...
    LogPrint("merchantnode", "CCommunityVoteManager::CheckAndRemove - PASSED\n");
}
...
void CCommunityVoteManager::NewBlock()
{
    ...
    if (merchantnodeSync.RequestedMerchantnodeAssets <= MERCHANTNODE_SYNC_COMMUNITYVOTE) return;

    // incremental sync with our peers
    if (merchantnodeSync.IsSynced()) {
        LogPrint("merchantnode", "CCommunityVoteManager::NewBlock - incremental sync started\n");
    ...
    LogPrint("merchantnode", "CCommunityVoteManager::NewBlock - askedForSourceProposalOrVote cleanup - size: %d\n", askedForSourceProposalOrVote.size());
    ...
    LogPrint("merchantnode", "CCommunityVoteManager::NewBlock - mapProposals cleanup - size: %d\n", mapProposals.size());
    std::map<uint256, CCommunityProposal>::iterator it2 = mapProposals.begin();
    while (it2 != mapProposals.end()) {
    ...
    LogPrint("merchantnode", "CCommunityVoteManager::NewBlock - vecCommunityProposals cleanup - size: %d\n", vecCommunityProposals.size());
    ...
    while (it4 != vecCommunityProposals.end()) {
        ...
        if (!(*it4).IsValid(strError)) {
            LogPrint("merchantnode", "mcprop - invalid community vote proposal - %s\n", strError);
        ...
        LogPrint("merchantnode", "mcprop - new community vote - %s\n", (*it4).GetHash().ToString());
        it4 = vecCommunityProposals.erase(it4);
    }

    LogPrint("merchantnode", "CCommunityVoteManager::NewBlock - PASSED\n");
}
...
void CCommunityVoteManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mncvs") { // Merchantnode community vote sync
        uint256 nProp;
        vRecv >> nProp;
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (nProp == 0) {
                if (pfrom->HasFulfilledRequest("mncvs")) {
                    LogPrint("merchantnode", "mncvs - peer already asked me for the list\n");
        ...
        Sync(pfrom, nProp);
        LogPrint("mncommunityvote", "mncvs - Sent Merchantnode community votes to peer %i\n", pfrom->GetId());
    }

    if (strCommand == "mcprop") { // Merchantnode Community Proposal
        CCommunityProposalBroadcast communityProposalBroadcast;
        vRecv >> communityProposalBroadcast;

        if (mapSeenMerchantnodeCommunityProposals.count(communityProposalBroadcast.GetHash())) {
            merchantnodeSync.AddedCommunityItem(communityProposalBroadcast.GetHash());
            return;
        }
        ...
        if (!IsCommunityCollateralValid(communityProposalBroadcast.nFeeTXHash, communityProposalBroadcast.GetHash(), strError, communityProposalBroadcast.nTime, nConf)) {
            LogPrint("merchantnode", "Community Proposal FeeTX is not valid - %s - %s\n", communityProposalBroadcast.nFeeTXHash.ToString(), strError);
            if (nConf >= 1) vecCommunityProposals.push_back(communityProposalBroadcast);
            return;
        }

        mapSeenMerchantnodeCommunityProposals.insert(make_pair(communityProposalBroadcast.GetHash(), communityProposalBroadcast));

        if (!communityProposalBroadcast.IsValid(strError)) {
            LogPrint("merchantnode", "mcprop - invalid community proposal - %s\n", strError);
            return;
        }
        ...
        LogPrint("merchantnode", "mcprop - new community - %s\n", communityProposalBroadcast.GetHash().ToString());

        // We might have active votes for this proposal that are valid now
        CheckOrphanVotes();
    }

    if (strCommand == "mcvote") { // Merchantnode Community Vote
        CCommunityVote vote;
        vRecv >> vote;
        vote.fValid = true;

        if (mapSeenMerchantnodeCommunityVotes.count(vote.GetHash())) {
            merchantnodeSync.AddedCommunityItem(vote.GetHash());
            return;
        }

        CMerchantnode* pmn = mnodeman.Find(vote.vin);
        if (pmn == NULL) {
            LogPrint("merchantnode", "mcvote - unknown merchantnode - vin: %s\n", vote.vin.prevout.hash.ToString());
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }

        mapSeenMerchantnodeCommunityVotes.insert(make_pair(vote.GetHash(), vote));
        if (!vote.SignatureValid(true)) {
            LogPrint("merchantnode", "mcvote - signature invalid\n");
            if (merchantnodeSync.IsSynced())
                Misbehaving(pfrom->GetId(), 20);
        ...
        LogPrint("merchantnode", "mcvote - new vote for community vote %s - %s\n", vote.nProposalHash.ToString(),  vote.GetHash().ToString());
    }
}
...
void CCommunityVoteManager::ResetSync()
{
    ...
    std::map<uint256, CCommunityProposalBroadcast>::iterator it1 = mapSeenMerchantnodeCommunityProposals.begin();
    while (it1 != mapSeenMerchantnodeCommunityProposals.end()) {
...
void CCommunityVoteManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    */
    ...
    pfrom->PushMessage("ssc", MERCHANTNODE_SYNC_COMMUNITYVOTE_PROP, nInvCount);
...
bool CCommunityVoteManager::UpdateProposal(CCommunityVote& vote, CNode* pfrom, std::string& strError)
{
    ...
    if (!mapProposals.count(vote.nProposalHash)) {
        if (pfrom) {
            ...
            if (!merchantnodeSync.IsSynced()) return false;

            LogPrint("merchantnode", "CCommunityVoteManager::UpdateProposal - Unknown proposal %d, asking for source proposal\n", vote.nProposalHash.ToString());
            mapOrphanMerchantnodeCommunityVotes[vote.nProposalHash] = vote;
...
// If merchantnode voted for a proposal, but is now invalid -- remove the vote
...
bool CCommunityVote::Sign(CKey& keyMerchantnode, CPubKey& pubKeyMerchantnode)
{
    // Choose coins to use
    ...
    if (!merchantnodeSigner.SignMessage(strMessage, errorMessage, vchSig, keyMerchantnode)) {
        LogPrint("merchantnode", "CCommunityVote::Sign - Error upon calling SignMessage");
        return false;
    }

    if (!merchantnodeSigner.VerifyMessage(pubKeyMerchantnode, vchSig, strMessage, errorMessage)) {
        LogPrint("merchantnode", "CCommunityVote::Sign - Error upon calling VerifyMessage");
        return false;
    }
...
bool CCommunityVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        LogPrint("merchantnode", "CCommunityVote::SignatureValid() - Unknown Merchantnode - %s\n", vin.prevout.hash.ToString());
        return false;
    }
    ...
    if (!merchantnodeSigner.VerifyMessage(pmn->pubKeyMerchantnode, vchSig, strMessage, errorMessage)) {
        LogPrint("merchantnode", "CCommunityVote::SignatureValid() - Verify message failed\n");
        return false;
    }
...
std::string CCommunityVoteManager::ToString() const
{
    ...
    info << "Proposals: " << (int)mapProposals.size() << ", Seen Community Proposals: " << (int)mapSeenMerchantnodeCommunityProposals.size() << ", Seen Community Votes: " << (int)mapSeenMerchantnodeCommunityVotes.size();
